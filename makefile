...
# Compiled libraries under user ~flexpart, gfortran v5.4
	ROOT_DIR = /home/zhendong/flexpart/flexpart_v10.4/flexpart_v10.4_3d7eebf/

	F90	  = /usr/bin/gfortran
	MPIF90    = /usr/bin/mpifort

	INCPATH   = /usr/include
	# INCPATH1  = /usr/local/nc/netcdf4/include

##Enable netCDF output
	NCOPT = -DUSE_NCF -lnetcdf -lnetcdff	

# path to gributils used to detect meteodata format
VPATH = gributils/
EMOSLIB=-lemosR64

LIBC = -L/usr/lib/x86_64-linux-gnu -L/usr/local/nc/hdf5/lib -lnetcdf -lhdf5_hl -lhdf5 -lpthread -lsz -lz -ldl -lm -lcurl
ECCODES_LIB= -L/usr/lib -L/usr/lib/x86_64-linux-gnu -leccodes_f90 -leccodes -lm
GRIB_API_LIB=  -Bstatic -Bdynamic -lm  
NC_LIB = -lnetcdf -lnetcdff -L/usr/lib -Wl,-Bsymbolic-functions -Wl,-z,relro -Wl,-z,now 
...
## LIBRARIES
LIBS = -lm $(LIBC) $(NC_LIB) $(GRIB_API_LIB) $(EMOSLIB) $(ECCODES_LIB)

FFLAGS   = -I$(INCPATH) -O$(O_LEV) -g -cpp -m64 -mcmodel=medium -fconvert=little-endian -frecord-marker=4 -fmessage-length=0 -flto=jobserver -O$(O_LEV) $(NCOPT) $(FUSER)  #-Warray-bounds -fcheck=all # -march=native
DBGFLAGS = -I$(INCPATH) -O$(O_LEV_DBG) -g3 -ggdb3 -cpp -m64 -mcmodel=medium -fconvert=little-endian -frecord-marker=4 -fmessage-length=0 -flto=jobserver -O$(O_LEV_DBG) $(NCOPT) -fbacktrace   -Wall  -fdump-core $(FUSER)  #  -ffpe-trap=invalid,overflow,denormal,underflow,zero  -Warray-bounds -fcheck=all

LDFLAGS  = $(FFLAGS) -L$(LIBPATH1) -Wl,-rpath, $(LIBS)
LDDEBUG  = $(DBGFLAGS) $(LIBS) 
